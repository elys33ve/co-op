...
lr_test(lr) -- returns gpio list for left or right side
...
pin_state(lr) -- list state of pins to display (by checking cmd line value) [unused rn; was messing up]
pin_state_inp(inp, lr, lf) -- same as pin_state() but lists pin state based on checkboxes [works but less accurate]
"""
############################################    ---     constants
...
def set_lvl (pins, lr):      # arg is list of gpio numbers (from input_value), right or left side
    ...
    for i in range(len(gpio)):
        ...
        if func == OUTPUT:
            ...
            # === ideally undo checked box
            # currently, it will show on the webserver that the level changed (switch on/off)
            # but only outputs can be changed from the cmd line
            pass
        ...
        if gpio[i] in pins and func != '-1':        # if pin checkboxed and is gpio -- set output
            if func == INPUT:
                if Test == True:
                    ...
                    lvl = pininfo_get(gpio[i])[0]                   # recheck lvl
                    # === update level
